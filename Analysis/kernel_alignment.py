...
if __name__ == "__main__":
    ...
    all_scores=[]
    if mode == "layer_wise":
        for layer_num in range(image_embeddings.shape[0]):
            feats_A = image_embeddings[layer_num]
            feats_B = text_embeddings[layer_num]
            feats_A = feats_A.to(torch.float32)
            feats_B = feats_B.to(torch.float32)
        ...
        pointwise_metric = {}
        for layer_num in range(image_embeddings.shape[0]):
            for layer_num_inner in range(image_embeddings.shape[0]):
                feats_A = image_embeddings[layer_num]
                feats_B = text_embeddings[layer_num_inner]
                feats_A = feats_A.to(torch.float32)
                feats_B = feats_B.to(torch.float32)
                feats_A = F.normalize(feats_A, dim=-1)
                feats_B = F.normalize(feats_B, dim=-1)
                ...
                for metric in metrics_we_care:
                    ...
                    for t in range(trials):
                        ...
                        score = AlignmentMetrics.measure(metric, feats_A, feats_B, **kwargs)
                        scores.append(score)
                        pointwise_metric[f"{layer_num}-{layer_num_inner}"] = score
                        times.append(time.time() - t_st)        
                    print(f"{metric.rjust(20)}: {np.mean(scores):1.3f} [elapsed: {np.mean(times):.2f}s]")
